...
- `orchestration_pipeline.md`：循环引擎、上下文组装、强制总结行为，以及带 MCP 感知的 prompt/tool 注入。
- `tool_runtime.md`：工具注册、执行器、`tool_run` 生命周期语义、动态 MCP tool/resource 表面，以及远程 SSH sandbox transport 行为。
...
- `ui_surfaces.md`：Web UI 与 TUI 的 setup 流程（`direct` 下本地/远程目录切换）、会话模式选择/锁定、MCP 选择状态、接口与 Tool/Steer Runs 面板。

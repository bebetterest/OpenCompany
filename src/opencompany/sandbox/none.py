...
class _InteractiveNoSandboxProcess(InteractiveSandboxProcess):
    def __init__(
        self,
        *,
...
class NoSandboxBackend(SandboxBackend):
    ...
    async def start_interactive(
        ...
        async def _read_stream(stream, channel: str, callback: ShellEventCallback | None) -> None:
            while True:
                line = await stream.readline()
                if not line:
                    break
        ...
        return _InteractiveNoSandboxProcess(
            backend=self,
            process=process,
